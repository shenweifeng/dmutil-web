    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        for (String key : WebsocketConstant.pushCtxMap.keySet()) {
            ...
            if (ctx.equals(WebsocketConstant.pushCtxMap.get(key))) {
                // 从连接池内剔除
                System.out.println("剔除前，连接数：" + WebsocketConstant.pushCtxMap.size());
                System.out.println("剔除" + key);
                WebsocketConstant.pushCtxMap.remove(key);
                System.out.println("剔除后，连接数：" + WebsocketConstant.pushCtxMap.size());
            }
    ...
    public void handlerWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        if (id != null && !"".equals("id") && type != null && !"".equals("type")) {
            ...
            if (idAccess && typeAccess) {
                ...
                WebsocketConstant.pushCtxMap.put(request, ctx);
                WebsocketConstant.aaChannelGroup.add(ctx.channel());
            }
